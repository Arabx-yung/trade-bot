...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target == "pending":
        ...
        if not rows:
            ...
            return
        keyboard = [
            [InlineKeyboardButton(f"{r[0]} | {r[1]} {r[2]} @ {r[3]}", callback_data=f"DELETE|PENDING|{r[0]}")]
            for r in rows
        ]
        await update.message.reply_text("Select a pending trade to delete:", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        if not rows:
            ...
            return
        keyboard = [
            [InlineKeyboardButton(f"{r[0]} | {r[1]} {r[2]} {r[3]}→{r[4]}", callback_data=f"DELETE|CLOSED|{r[0]}")]
            for r in rows
        ]
        await update.message.reply_text("Select a closed trade to delete:", reply_markup=InlineKeyboardMarkup(keyboard))

    else:
        await update.message.reply_text("Invalid argument. Use /delete pending OR /delete closed")
    conn.close()
...
async def delete_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("DELETE|PENDING|"):
        trade_id = query.data.split("|")[2]
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("DELETE FROM pending_trades WHERE trade_id = ?", (trade_id,))
        conn.commit()
        conn.close()
        await query.edit_message_text(f"✅ Pending trade {trade_id} deleted.")

    elif query.data.startswith("DELETE|CLOSED|"):
        trade_id = query.data.split("|")[2]
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        await query.edit_message_text(f"✅ Closed trade {trade_id} deleted.")
